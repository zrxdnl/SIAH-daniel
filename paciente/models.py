...
class Paciente(models.Model):
     ...
     cartao_sus_paciente = models.CharField(max_length=15, unique = True, error_messages={"unique":"Cartão do SUS já cadastrado, insira outro"}, validators = [validate_digits, validate_sus])
     numero_do_documento_paciente = models.CharField(primary_key=True, max_length = 11, unique = True, error_messages={"unique":"Número de CPF já cadastrado, insira outro."}, validators = [validate_cpf, validate_digits])
     contato_emergencia_paciente = models.CharField(max_length=11, validators = [validate_digits, validate_contato])

     def __str__(self):
          return self.numero_do_documento_paciente
