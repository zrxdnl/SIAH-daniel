...
class ListaPacienteView(ListView):
     ...
     def fichaPaciente(self, valor):
          queryset = Fichamedica.filter(Numero_CPF__cpf_paciente__icontains=valor)
          return queryset
     ...
     def get_queryset(self):
          ...
          if filtro3:
               queryset = Prontuario.objects.filter(Numero_CPF__cpf_paciente__icontains=filtro3)

          if filtro2:
               queryset = Fichamedica.objects.filter(Numero_CPF__cpf_paciente__icontains=filtro2)
         
          if filtro:
               if filtro.isdigit():
                    queryset = queryset.filter(cpf_paciente__icontains=filtro)
               else:
                    queryset = queryset.filter(Nome__contains=filtro)
          return queryset
